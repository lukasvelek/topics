function handleFilterCategoryChange() {
    ...
    $.get(
        "?page=AdminModule:FeedbackSuggestions&action=getFilterCategorySuboptions&isAjax=1",
        {
            category: select
        }
    )
...
async function handleGridFilterChange() {
    ...
    await getSuggestionsGrid(-1, type, key);
}
...
async function handleGridFilterClear() {
    ...
    await getSuggestionsGrid(-1, 'null', 'null');
}